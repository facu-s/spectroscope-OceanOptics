...
def init_db(app):
    ...
    c = conn.cursor()
    try:
        c.execute('''create table spectro_scope (CREATE TABLE spectro_scope
                                                 (
    ...
    with app.app_context():
        ...
        for variable in configuration_mapping.keys():
            if spectro_scope_config[variable]:
                ...
                string_sql = sql_stat_build(string_sql, config_str, value_control, l_spectro_scope, spectro_pointer_config[variable])
                value_control+=1
